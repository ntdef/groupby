...
fn main() {
    let args = App::new("groupby")
        ...
        .arg(Arg::with_name("poolsize")
             .short("p")
             .long("pool-size")
             .help("The number of workers to allow (default is 8).")
             .takes_value(true))
        .arg(Arg::with_name("skiplines")
             .long("skip")
             .help("Number of lines to skip (default is 0).")
             .takes_value(true))
        .arg(Arg::with_name("key")
             .short("k")
             .long("key")
             .value_name("KEYCOLS")
             .help("Index of column to use as the grouping key (1-indexed; default is 1)")
             .takes_value(true))
        .arg(Arg::with_name("delimiter")
             .short("d")
             .long("delimiter")
             .value_name("DELIMITER")
             .help("The field separator (default is ',').")
             .takes_value(true))
        .arg(Arg::with_name("command")
             .value_name("COMMAND")
             .help("The command to run on each group (required)")
             .required(true)
             .takes_value(true)
             .index(1))
        .arg(Arg::with_name("input")
             .help("The input file to pass (default is stdin)")
             .value_name("INPUT")
             .index(2))
        .get_matches();
